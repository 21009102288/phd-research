...
[[空间分辨光谱_SRS]] 使用多个源探距的衰减斜率估计组织吸收特性。它和 MBLL 的核心差别是：MBLL 用时间上的光强变化 $\Delta A$ 推浓度变化，SRS 用空间上的衰减斜率 $\partial A/\partial \rho$ 推吸收相关量，从而更适合输出绝对或准绝对组织氧合指数。

Suzuki 等 NIRO-300 论文给出的推导路径如下。先从强散射半无限介质的扩散近似出发，令 $\rho$ 为源探距，$R(\rho,t)$ 为脉冲输入后的反射光时间分布，$\mu_a$ 为吸收系数，$\mu_s'$ 为约化散射系数：

$$
I(\rho)=\int_0^\infty R(\rho,t)dt
$$

定义空间位置 $\rho$ 处的衰减：

$$
A(\rho,\lambda)=-\log_{10} I(\rho,\lambda)
$$

在距离光源足够远、扩散近似成立时，衰减对距离的斜率可写成吸收和散射的函数：

$$
S(\rho,\lambda)=\frac{\partial A(\rho,\lambda)}{\partial \rho}
\approx
\frac{1}{\ln 10}
\left[
\sqrt{3\mu_a(\lambda)\mu_s'(\lambda)}
+\frac{2}{\rho}
\right]
$$

因此 SRS 的测量对象不是单点光强，而是多距离探测器估计出的 $S(\rho,\lambda)$。把上式反解可得到：

$$
\mu_a(\lambda)\mu_s'(\lambda)
\approx
\frac{1}{3}
\left[
\ln 10 \cdot S(\rho,\lambda)-\frac{2}{\rho}
\right]^2
$$

如果在近红外窗口内先把约化散射的波长依赖写成：

$$
\mu_s'(\lambda)=k(1-h\lambda)
$$

其中 $k$ 是未知散射尺度，$h$ 是散射随波长下降的归一化斜率，则可得到：
...
这一步是 SRS 能输出 TOI 的关键：它不一定先求出绝对 $\mu_a$ 和绝对 $\mu_s'$，而是求出带同一个比例因子 $k$ 的吸收谱 $k\mu_a(\lambda)$。再利用血红蛋白吸收模型：

$$
\mu_a(\lambda)
=
\varepsilon_{HbO}(\lambda)HbO
+
\varepsilon_{HbR}(\lambda)HbR
+
\mu_{a,baseline}(\lambda)
$$

在多个波长下用最小二乘估计 $kHbO$ 和 $kHbR$。由于 TOI 是比值，公共比例因子 $k$ 抵消：
...
所以 SRS/TOI 的逻辑是：多距离光强 $\rightarrow$ 空间斜率 $S$ $\rightarrow$ 相对吸收谱 $k\mu_a$ $\rightarrow$ 相对 HbO/HbR $\rightarrow$ 比值型氧合指数 TOI。NIRO-300 使用多波长光源和三段探测器在约 5 cm 源探距附近测量局部斜率，并持续检查 $A(\rho)$ 对 $\rho$ 的线性，这是因为 SRS 假设测量区间内衰减斜率稳定。

Matcher、Cope 和 Delpy 的水吸收谱论文为 SRS 定量化提供了另一条旁证：水是组织中相对稳定且浓度可估的内源参考色团，820 nm 和 975 nm 附近水吸收特征可用于估计 differential pathlength。Matcher 和 Cooper 的脱氧血红蛋白绝对定量论文进一步说明，若能用水吸收作为参考，就可以通过二阶微分光谱估计 Hb 相对水的浓度，并转成绝对 Hb 浓度。它们不等同于 SRS，但共同说明一个原则：CW-NIRS 若要从“趋势监测”走向“准绝对/绝对定量”，必须引入额外约束，例如空间斜率、已知参考色团、散射先验或 TD/FD 光程信息。

SRS 常见于临床脑氧监测产品，因为它比单距离 CW 更适合估计绝对或准绝对组织氧合。以 Hamamatsu NIRO-300 的公开论文为例，其验证路径包括血液/Intralipid 仿体与血气分析仪对比，以及人体前臂与 TRS 系统同步测量。该证据支持 SRS 可作为床旁组织氧合指数方案，但其准确性仍受以下因素限制：

- 扩散近似要求源探距足够大，且测量区间内局部组织可近似均匀。
- $A(\rho)$ 对 $\rho$ 的线性是 SRS 成立的工程前提，探测器需要验证局部斜率稳定。
- $\mu_s'(\lambda)=k(1-h\lambda)$ 是先验近似，不同组织、年龄和脑区可能改变散射谱。
- 多层头部结构会让头皮、颅骨、脑脊液和皮层对斜率的贡献混合。
- TOI 是比值型准绝对指标，不等同于 HbO/HbR 的完整绝对浓度。
...
| 论文/资料                                 | 对应章节        | 技术特点/创新点                                        | 价值贡献                             | 当前使用方式                 |
| ------------------------------------- | ----------- | ----------------------------------------------- | -------------------------------- | ---------------------- |
| [[Jöbsis_1977_NIRS起点]]                | 1.1、9.1     | 提出近红外组织穿透窗口和无创氧合监测设想                            | 奠定 NIRS 用于脑/心肌氧合监测的概念起点          | 支撑基本原理，不直接支撑现代 rSO2 算法 |
| [[Delpy_1988_DPF光程测量]]                | 1.3、5.4     | 用皮秒飞行时间测量平均光程，引出 DPF 定量基础                       | 说明组织强散射导致实际光程远长于源探距              | 支撑 MBLL、DPF、验证体系       |
| Cope & Delpy 1988 新生儿系统               | 1.8、5.4     | 在高背景衰减下实现长期新生儿脑 NIRS 透射监测                       | 把 NIRS 从概念推进到床旁工程系统，强调动态范围、漂移和安全 | 支撑工程验证指标               |
| [[Patterson_1989_TD组织光学参数]]           | 1.5、5.4     | 建立时间分辨脉冲形状与 $\mu_a$、$\mu_s'$ 的关系                | 证明 TD 可作为绝对定量和深度信息的理论上限          | 作为 CW/SRS 校准参考         |
| Hiraoka et al. 1993 非均质光程             | 1.3、5.3、7.3 | 引入 partial differential pathlength，分析多层组织贡献     | 证明浅层污染和多层头模型是物理问题，不只是算法后处理       | 支撑个体化 DPF 与多层头模型课题     |
| [[Matcher_1994_水吸收DPF估计]]             | 1.4、5.4     | 用水吸收谱估计 differential pathlength                    | 提供 CW 条件下估计光程/定量尺度的参考色团方法        | 支撑 SRS 定量化旁证           |
| [[Matcher_Cooper_1994_脱氧血红蛋白绝对定量]] | 1.4、1.8     | 用二阶微分光谱估计 Hb 相对水的浓度比                              | 展示水参考色团可支持脱氧血红蛋白绝对定量             | 支撑 TOI 与绝对 Hb 浓度边界     |
| Wang/Jacques/Zheng 1995 MCML          | 5.3         | 开源 ANSI C 多层组织蒙特卡洛模型                            | 提供可复现光传输仿真范式                     | 作为 MCX/NIRFAST 前的经典基线  |
| Boas et al. 2002 tMCimg               | 5.3、7.3     | 支持 3D 非均质介质与 MRI/CT 结构先验                        | 把蒙特卡洛推进到真实头模型 forward problem    | 支撑个体化头模型仿真路线           |
| [[Suzuki_2003_NIRO300_SRS_TOI]]       | 1.4、2.5、5.4 | 用 SRS 空间斜率输出 TOI，并用仿体血气/TRS 对照验证                | 证明 CW-SRS 可形成临床床旁组织氧合指标          | 支撑低成本 CW-SRS 原型方向      |
| Kocsis et al. 2006 MBLL revisited     | 1.6、4.1     | 讨论 dMBLL 的均匀吸收与散射不变假设                           | 指出散射变化和 partial volume 会造成浓度估计误差 | 支撑算法误差边界分析             |
| Huppert et al. 2009 HomER             | 4.2、4.7     | 系统总结 fNIRS 时间序列处理、伪迹和统计分析                       | 提供从原始 OD 到任务响应分析的软件流程参考          | 支撑工具链和预处理规范            |
| Saager et al. 2011 C-NIRS             | 4.4、7.3     | 用短距探测器记录 scalp-only 趋势并校正长距通道                   | 证明双探测器可提高脑源响应检出率和稳定性             | 支撑短距通道回归方向             |
| Kacprzak et al. 2012 颈动脉手术 TD 成像      | 5.6、7.3     | 用 TD 光学脑成像区分颅内/颅外血氧变化                           | 展示 TD 在术中脑灌注监测中的临床潜力             | 支撑临床验证场景               |
| [[Ferrari_Quaresima_2012_fNIRS历史综述]]  | 2.5、7.2     | 梳理 fNIRS 从单点到多通道、无线/可穿戴的发展                      | 提供技术路线和应用场景历史框架                  | 支撑研究背景                 |
| Scholkmann & Wolf 2013 DPF 方程         | 1.3、7.3     | 建立额叶 DPF 与年龄、波长的经验方程                            | 把固定 DPF 推进为可参数化先验                | 支撑个体化 DPF 校正           |
| Gagnon et al. 2014 双短距回归              | 4.4、7.3     | 在 source 和 detector 两端同时布置短距通道                  | 比单短距更有效降低浅层系统性干扰                 | 支撑探头布局优化               |
| Strangman et al. 2014 Colin27 头模型     | 5.3、7.3     | 用 3555 次蒙特卡洛评估头皮/颅骨厚度影响                         | 量化不同头区和源探距下灰质灵敏度                 | 支撑源探距和测量脑区选择           |
| [[Scholkmann_2014_CW_fNIRS综述]]        | 2.5、4.7、7.2 | 总结 CW-fNIRS 仪器、探头、MBLL 和分析方法                    | 是低成本 CW/fNIRS 工程调研主索引            | 支撑架构与方法学               |
| Pifferi et al. 2016 TD diffuse optics | 1.5、7.3     | 总结 time-gating、null distance、SPAD/SiPM 等 TD 新器件 | 指出 TD 小型化和可穿戴化的技术路径              | 支撑长期高阶路线               |
| Davies et al. 2017 FD vs CW 临床低氧      | 2.1、5.6     | 在受控低氧中比较临床可用 FD 与 CW 设备                         | 提醒 FD 理论优势不必然转化为临床监测优势           | 支撑设备实证比较               |
| Lange & Tachtsidis 2019 TD 临床综述       | 1.5、5.6、7.2 | 汇总 TD-NIRS 在新生儿、卒中、TBI、术中等场景应用                  | 明确 TD 的绝对值、散射和脑特异性优势             | 支撑 TD 作为理论上限和验证参考      |
| Fantini & Sassaroli 2020 FD 综述        | 1.5、2.5、7.2 | 系统总结 FD 的 DC/AC/相位信息和脑敏感性                       | 明确 FD 在绝对光学参数和深层敏感性上的价值          | 支撑 FD 与 CW 的取舍         |
...
| 论文 | 技术特点/创新点 | 对算法链路的价值 | 建议落地方式 |
|---|---|---|---|
| Kocsis et al. 2006 MBLL revisited | 把 dMBLL 的误差来源拆成非均匀吸收、色团串扰和散射变化串扰 | 提醒 OD 到 HbO/HbR 不是简单矩阵求逆；散射变化会伪装成浓度变化 | 在算法章节增加误差预算，仿真不同 $\Delta\mu_s'$ 对 HbO/HbR 的偏差 |
| Huppert et al. 2009 HomER | 给出 fNIRS 时间序列处理、运动伪迹处理、GLM/统计分析的实用框架 | 把“采集—预处理—统计响应”串成可复现流程 | 作为 Python/MATLAB 工具链的流程模板 |
| Saager et al. 2011 C-NIRS | 用短距通道测量浅层血流趋势并从长距通道中扣除 | 证明短距校正可提高脑源响应检出率和单人响应稳定性 | 原型设计至少预留一个短距通道 |
| Gagnon et al. 2014 双短距回归 | 在 source 和 detector 两端同时布置短距测量，刻画浅层干扰的空间非均质性 | 相比单短距通道，双短距对 HbO/HbR 噪声抑制更强 | 探头布局优先考虑 source-side 与 detector-side 短距冗余 |
| Strangman et al. 2014 Colin27 仿真 | 定量评估头皮/颅骨厚度、头区和源探距对灰质灵敏度的影响 | 说明算法必须绑定头区、源探距和个体解剖差异 | 用蒙特卡洛生成通道级灵敏度先验 |
...
| 专利 | 技术特点/创新点 | 价值贡献 | 对自研/规避的启发 |
|---|---|---|---|
| `US 5,465,714` | 柔性电光传感器、光源/探测器独立安装座、导线屏蔽、柔软外覆层 | 把“可贴附、可重复稳定定位”的传感器结构产品化，是脑氧仪耗材壁垒的早期样本 | 自研探头要重点区分机械定位、遮光、贴附材料和电连接方案 |
| `US 8,644,901 B2` | 四波长、近/远探测器、OD 空间对比、一阶/二阶 rSO2 估计比较 | 把异常人群/异常光学路径识别纳入 rSO2 计算流程，提高临床鲁棒性 | 不能只做双波长 MBLL；需要质量门控、异常值识别和多路径估计交叉验证 |
| `US 9,161,722 B2` | 保留光电核心，更换患者接触层/衬垫层，覆盖再制造流程 | 保护耗材生命周期和商业闭环，说明知识产权不只在测量算法 | 自研产品若有一次性耗材，要提前设计可制造、可追溯、可合规的耗材策略 |
...
| 专利 | 技术特点/创新点 | 价值贡献 | 对自研/规避的启发 |
|---|---|---|---|
| `CN 113288137 A` | 锥形凸起、聚光透镜、硅胶壳体、光源与双传感器布局 | 面向有发头皮区域，解决头发遮挡和光耦合不稳定 | 可比较拨发结构、柔性材料、光窗形状和传感器贴合方式 |
| `CN 111466922 A` | 柔性衬板、多光电传感器直线排布、有效数据通道选择 | 用多距离/多通道冗余适配不同头型、组织厚度和贴合质量 | 自研可采用不同几何或不同质量评价策略，避免落入必要技术特征组合 |
| `CN 115844392 A` | 前额叶、枕叶、顶叶、颞叶多脑区无线穿戴式系统 | 将脑氧监测从单前额叶扩展到多脑区覆盖，服务卒中/后循环场景 | 需要关注多探头同步、无线延迟、佩戴定位和跨脑区校准 |
| `CN 106580248 A` | EEG 事件诱发电位与 fNIRS 脑血氧响应函数耦合分析 | 提供神经血管耦合分析方法线索，不是中科搏锐自有专利样本 | 可作为多模态算法背景，不能作为中科搏锐产品权利归属证据 |
| `CN 116269365 B` | 每个通道含一个光源和多组近端/远端传感器 | 用冗余近/远端阵列缓解头发遮挡或局部传感器失效 | 若做近/远端布局，需重点核查其独立权利要求的传感器组合方式 |
| `CN 219126358 U` | 壳体、采集处理模块、无线模块、加速度传感器、遮光垫片集成 | 从光学测量扩展到完整穿戴式采集终端 | 自研要区分探头结构、电子模块位置、IMU 用途和遮光垫片结构 |
| 电子科大 2015 局部脑组织血氧血容绝对量检测装置及方法 | 从标题看聚焦局部脑组织血氧/血容绝对量检测 | 可作为国内绝对定量路线的补充线索，需进一步核验权利要求和法律状态 | 暂不用于 FTO 判断；后续应补充公开号、申请人和独权拆解 |
...
- Patterson, M. S., Chance, B., & Wilson, B. C. (1989). *Time resolved reflectance and transmittance for the non-invasive measurement of tissue optical properties*. Applied Optics, 28(12), 2331–2336. DOI: `10.1364/AO.28.002331`
- Hiraoka, M., Firbank, M., Essenpreis, M., Cope, M., Arridge, S. R., van der Zee, P., & Delpy, D. T. (1993). *A Monte Carlo investigation of optical pathlength in inhomogeneous tissue and its application to near-infrared spectroscopy*. Physics in Medicine and Biology, 38(12), 1859–1876.
- Matcher, S. J., Cope, M., & Delpy, D. T. *Use of the water absorption spectrum to quantify tissue chromophore concentration changes in near-infrared spectroscopy*.
- Matcher, S. J., & Cooper, C. E. (1994). *Absolute quantification of deoxyhaemoglobin concentration in tissue near infrared spectroscopy*. Physics in Medicine and Biology, 39, 1295–1312.
- Suzuki, S., Takasaki, S., Ozaki, T., & Kobayashi, Y. (2003). *A tissue oxygenation monitor using NIR spatially resolved spectroscopy*. Proceedings of SPIE.
- Scholkmann, F., & Wolf, M. (2013). *General equation for the differential pathlength factor of the frontal human head depending on wavelength and age*. Journal of Biomedical Optics, 18(10), 105004.
- Scholkmann, F., Kleiser, S., Metz, A. J., et al. (2014). *A review on continuous wave functional near-infrared spectroscopy and imaging instrumentation and methodology*. NeuroImage, 85, 6–27. DOI: `10.1016/j.neuroimage.2013.05.004`
- Ferrari, M., & Quaresima, V. (2012). *A brief review on the history of human functional near-infrared spectroscopy development and fields of application*. NeuroImage, 63(2), 921–935. DOI: `10.1016/j.neuroimage.2012.03.049`
...
- Boas, D. A., Culver, J. P., Stott, J. J., & Dunn, A. K. (2002). *Three dimensional Monte Carlo code for photon migration through complex heterogeneous media including the adult human head*. Optics Express.
- Kocsis, L., Herman, P., & Eke, A. (2006). *The modified Beer–Lambert law revisited*. Physics in Medicine and Biology, 51, N91–N98.
- Huppert, T. J., Diamond, S. G., Franceschini, M. A., & Boas, D. A. (2009). *HomER: a review of time-series analysis methods for near-infrared spectroscopy of the brain*. Applied Optics.
- Saager, R. B., Telleri, N. L., & Berger, A. J. (2011). *Two-detector corrected near infrared spectroscopy detects hemodynamic activation responses more robustly than single-detector NIRS*. NeuroImage.
- Kacprzak, M., Liebert, A., Staszkiewicz, W., et al. (2012). *Application of a time-resolved optical brain imager for monitoring cerebral oxygenation during carotid surgery*. Journal of Biomedical Optics, 17(1), 016002.
- Gagnon, L., Yücel, M. A., Boas, D. A., & Cooper, R. J. (2014). *Further improvement in reducing superficial contamination in NIRS using double short separation measurements*. NeuroImage.
- Strangman, G. E., Zhang, Q., & Li, Z. (2014). *Scalp and skull influence on near infrared photon propagation in the Colin27 brain template*. NeuroImage.
- Pifferi, A., Contini, D., Dalla Mora, A., et al. (2016). *New frontiers in time-domain diffuse optics, a review*. Journal of Biomedical Optics, 21(9), 091310.
- Davies, D. J., Clancy, M., Lighter, D., et al. (2017). *Frequency-domain vs continuous-wave near-infrared spectroscopy devices: a comparison of clinically viable monitors in controlled hypoxia*. Neurocritical Care.
